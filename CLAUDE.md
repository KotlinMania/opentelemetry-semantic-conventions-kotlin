# Claude Code Project Instructions — opentelemetry-semantic-conventions-kotlin

## Project Overview

This is **opentelemetry-semantic-conventions-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`opentelemetry-semantic-conventions`](https://crates.io/crates/opentelemetry-semantic-conventions).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/opentelemetry-semantic-conventions/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/opentelemetry-semantic-conventions>
