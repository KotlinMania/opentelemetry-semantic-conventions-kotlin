...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `OpentelemetrySemanticConventions.swiftmodule/`
...
//      `import OpentelemetrySemanticConventions` above would fail at compile time.
//
//   2. The static archive `libOpentelemetrySemanticConventions.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "OpentelemetrySemanticConventions" }` and
//      `flattenPackage = "io.github.kotlinmania.opentelemetrysemanticconventions"` configuration in
...
//      via the `OpentelemetrySemanticConventionsLibrary` product.
//
...
final class OpentelemetrySemanticConventionsExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "OpentelemetrySemanticConventions swift module imported cleanly")
    }
}